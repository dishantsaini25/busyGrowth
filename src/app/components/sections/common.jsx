export const Chip = ({ children, variant = "default", className = "" }) => {
  ...
  return <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 font-medium ${v[variant]} ${className}`}>{children}</span>;
};
...
export const Metric = ({ label, value, tone = "neutral" }) => {
  ...
  return (
    <div className="flex flex-col">
      <span className="text-[10px] uppercase tracking-wider text-text-light xl:text-xs">{label}</span>
      <span className={`text-base font-bold xl:text-lg ${t[tone]}`}>{value}</span>
    </div>
  );
};